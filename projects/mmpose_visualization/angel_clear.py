#!/usr/bin/env python3
"""多人关节角度计算与可视化（CPU / 清爽版）。

基于 angel_multi.py：对每个人分别算肘/膝/髋角度并叠加骨架与关节文字，
但不绘制左上角黑色汇总面板。

默认轻量模型 RTMDet-nano + RTMPose-t，device=cpu。
...
用法（在仓库根目录 mmpose_CSlab2/ 下执行）：
  ...
  python projects/mmpose_visualization/angel_clear.py \\
      --input path/to/video.mp4 --output-dir projects/mmpose_visualization/outputs --stride 2
"""
...
def detect_and_pose_multi(
    ...
    order = np.argsort(-mean_scores)  # 高分优先
...
def plot_angle_curves(
    ...
    out_path: Path,
    fps: float = 30.0,
    title: str = "Joint Angle over Time (multi)",
) -> None:
...
def process_video(args, detector, pose_estimator) -> None:
    ...
    # 曲线默认跟踪前 curve_people 个人（按每帧分数排序后的 P0..）
    ...
    while True:
        ...
        if inferred % 10 == 0 or (need_infer and inferred == 1):
            print_log(
    ...
    plot_angle_curves(
        ...
        fps=fps,
        title="Joint Angle over Time (multi-person)",
    )
    ...
    with open(stats_path, "w", encoding="utf-8") as f:
        f.write("Joint angle statistics (multi-person CPU, clear/no panel)\n")
        f.write("=" * 40 + "\n")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Multi-person RTMPose joint angle (CPU, no panel)"
    )
    parser.add_argument("--input", type=str, required=True)
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(ROOT / "angle_results_clear"),
    )
    parser.add_argument(
        "--det-config",
        type=str,
        default=str(
            MMPOSE_ROOT / "demo/mmdetection_cfg/rtmdet_nano_320-8xb32_coco-person.py"
        ),
    )
    parser.add_argument(
        "--det-checkpoint",
    ...
    parser.add_argument("--max-people", type=int, default=10, help="最多处理人数")
    parser.add_argument(
        "--curve-people",
        type=int,
        default=2,
        help="曲线跟踪前 N 个人（P0..P{N-1}）",
    )
...
def main():
    ...
    print_log("Mode: multi-person clear / no panel (CPU)", logger="current")
